...
class DisplayNews {
    ...
    void draw(json &jsonData, int current_element, bool refreshed, int max,
        ...
        for (int i = current_element_top_visible; i < jsonData["articles"].size();
                i++) {
            ...
            if (jsonData["articles"][i]["source"]["name"].is_null() ||
                    jsonData["articles"][i]["publishedAt"].is_null() ||
                    jsonData["articles"][i]["title"].is_null())
      ...
      */
      ...
      std::string sourceName =
          jsonData["articles"][i]["source"]["name"].get<std::string>();
      ...
      addstr((format_datetime(
                  jsonData["articles"][i]["publishedAt"].get<std::string>()))
                 .c_str());
      attroff(A_BOLD);
      addstr(" | ");
